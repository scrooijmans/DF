...
export function createTableViewStore(tableId: string) {
  ...
  const rowCount = derived(
    filteredRows,
    ($filteredRows) => $filteredRows.total
  );
...
export function createDashboardStore(dashboardId: string) {
  ...
  const metrics = derived(
    [config, tableStore.changeNotifier],
    ([$config]) => {
...
const userTableViewStore = derived(
  tableStore.changeNotifier,
  ($change) => {
...
const filteredTableStore = derived(
  tableStore,
  ($store) => applyFilters($store.getRows())
);
...
export function createViewStore(viewId: string, config: ViewConfig) {
  ...
  return derived(
    baseStore.changeNotifier,
    () => transformData(baseStore.getData(), config)
  );
}
