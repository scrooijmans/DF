...
```
123456789async function signUpNewUser() {  const { data, error } = await supabase.auth.signUp({    email: '[email protected]',    password: 'example-password',    options: {      emailRedirectTo: 'https://example.com/welcome',    },  })}
```
...
```
123456async function signInWithEmail() {  const { data, error } = await supabase.auth.signInWithPassword({    email: '[email protected]',    password: 'example-password',  })}
```
...
```
123await supabase.auth.resetPasswordForEmail('[email protected]', {  redirectTo: 'http://example.com/account/update-password',})
```
