...
```
12345678910async function signInWithEmail() {  const { data, error } = await supabase.auth.signInWithOtp({    email: '[email protected]',    options: {      // set this to false if you do not want the user to be automatically signed up      shouldCreateUser: false,      emailRedirectTo: 'https://example.com/welcome',    },  })}
```
...
```
1234const { error } = await supabase.auth.verifyOtp({  token_hash: 'hash',  type: 'email',})
```
...
```
1234567const { data, error } = await supabase.auth.signInWithOtp({  email: '[email protected]',  options: {    // set this to false if you do not want the user to be automatically signed up    shouldCreateUser: false,  },})
```
...
```
12345678const {  data: { session },  error,} = await supabase.auth.verifyOtp({  email: '[email protected]',  token: '123456',  type: 'email',})
```
