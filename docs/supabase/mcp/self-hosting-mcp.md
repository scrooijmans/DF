...
Comment out the request-termination section
Remove the # symbols from the entire section starting with - name: cors, including deny: []
Add your local IP to the 'allow' list.
Your edited configuration should look like the example below.
...
- name: mcp
  ...
  strip_path: true
  paths: - /mcp
  plugins: # Block access to /mcp by default
  #- name: request-termination # config: # status_code: 403 # message: "Access is forbidden." # Enable local access (danger zone!) # 1. Comment out the 'request-termination' section above # 2. Uncomment the entire section below, including 'deny' # 3. Add your local IPs to the 'allow' list - name: cors - name: ip-restriction
  config:
  allow: - 127.0.0.1 - ::1 # Add your Docker bridge gateway IP below - 172.18.0.1 # Do not remove deny!
  deny: []
...
Update Kong configuration file to the latest version and edit carefully
Confirm the Docker bridge gateway IP is correctly added in ./volumes/api/kong.yml
Check Kong's logs for errors: docker compose logs kong
