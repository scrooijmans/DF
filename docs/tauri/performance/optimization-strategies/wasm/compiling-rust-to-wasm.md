Title: Compiling from Rust to WebAssembly - WebAssembly | MDN

Description: If you have some Rust code, you can compile it into WebAssembly (Wasm). This tutorial will show you how to compile a Rust project into WebAssembly and use it in an existing web app.
...
Our Rust code has three main parts; let's talk about each one in turn. We give a high-level explanation here, and gloss over some details; to learn more about Rust, please check the free online book The Rust Programming Language.
...
Whenever you want to call JavaScript functions, you can add them to this file, and `wasm-bindgen` sets everything up for you. Not everything is supported yet, but we're working on it. Please file bugs if something is missing.
...
Now that we've completed the set up, let's build the package. We'll be using the generated code in a native ES module and Node.js. For this purpose, we'll use the `--target` argument in `wasm-pack build` to specify what kind of WebAssembly and JavaScript is generated.
...
This does several things. To learn about them in detail, check out this blog post on Mozilla Hacks. In short, `wasm-pack build`:
...
Serve the project root with a local web server, (e.g., `python3 -m http.server`). If you're not sure how to do that, refer to Running a simple local HTTP server.

**Note:** Use an up-to-date web server that supports the `application/wasm` MIME type. Older web servers might not support it yet.
...
To get Node.js and npm, go to the Get npm! page and follow the instructions. This tutorial targets node 20. To switch between node versions, you can use nvm.

To use the WebAssembly module with npm, we'll need to make a few changes. Let's start by recompiling our Rust with `bundler` option as the target:
...
Let's create a new directory inside the `hello-wasm` directory called `site` to try it out. We haven't published the package to the npm registry yet, so we can install it from a local version using `npm i /path/to/package`. You may use `npm link`, but installing from a local path is convenient for this demo:
...
Learn how to contribute.This page was last modified on May 30, 2025 by MDN contributors.
