...
![Example parquet file shown visually. The parquet file (taxi.parquet) is divided into row groups that each have two columns (pickup_at and dropoff_at)](https://duckdb.org/images/blog/parquet.svg "Taxi Parquet File")
...
## [DuckDB and Parquet](#duckdb-and-parquet)
...
## [Automatic Filter & Projection Pushdown](#automatic-filter--projection-pushdown)
...
![Projection & filter pushdown into parquet file example.](https://duckdb.org/images/blog/parquet-filter-svg.svg "Filter Pushdown")
...
## [DuckDB versus Pandas](#duckdb-versus-pandas)
...
## [Reading Multiple Parquet Files](#reading-multiple-parquet-files)

First we look at some rows in the dataset. There are three Parquet files in the `taxi/` folder. [DuckDB supports the globbing syntax](https://duckdb.org/docs/stable/data/parquet/overview.html), which allows it to query all three files simultaneously.
...
## [Concatenate into a Single File](#concatenate-into-a-single-file)
...
Note that [DuckDB also has support for writing Parquet files](about:/docs/stable/data/parquet/overview.html#writing-to-parquet-files) using the COPY statement.

## [Querying the Large File](#querying-the-large-file)
