# Integration Testing — Apache Arrow v21.0.0
...
The integration tests exercise compliance with several Arrow specifications: the [IPC format](about:blank/Columnar.html#format-ipc), the [Flight RPC](about:blank/Flight.html#flight-rpc) protocol, and the [C Data Interface](about:blank/CDataInterface.html#c-data-interface).

Strategy[#](#strategy "Permalink to this heading")
--------------------------------------------------

Our strategy for integration testing between Arrow implementations is:

*   Test datasets are specified in a custom human-readable, [JSON-based format](#format-json-integration) designed exclusively for Arrow’s integration tests.
...
### Example: IPC format[#](#example-ipc-format "Permalink to this heading")
...
### Example: C Data Interface[#](#example-c-data-interface "Permalink to this heading")

Now, let’s say we are testing Arrow Go as a producer and Arrow C# as a consumer of the Arrow C Data Interface.

1.  The integration testing harness allocates a C [ArrowArray](about:blank/CDataInterface.html#c-data-interface-struct-defs) structure on the heap.
    
2.  A Go in-process entrypoint (for example a C-compatible function call) reads a JSON file and exports one of its [record batches](about:blank/Glossary.html#term-record-batch) into the `ArrowArray` structure.
...
Running integration tests[#](#running-integration-tests "Permalink to this heading")
------------------------------------------------------------------------------------

The integration test data generator and runner are implemented inside the [Archery](about:blank/developers/continuous_integration/archery.html#archery) utility. You need to install the `integration` component of archery:
...
See [Running Docker Builds](about:blank/developers/continuous_integration/docker.html#docker-builds) for more information about the project’s `docker compose` configuration.

JSON test data format[#](#json-test-data-format "Permalink to this heading")
----------------------------------------------------------------------------

A JSON representation of Arrow columnar data is provided for cross-language integration testing purposes. This representation is [not canonical](https://lists.apache.org/thread.html/6947fb7666a0f9cc27d9677d2dad0fb5990f9063b7cf3d80af5e270f%40%3Cdev.arrow.apache.org%3E) but it provides a human-readable way of verifying language implementations.

See [here](https://github.com/apache/arrow/tree/main/docs/source/format/integration_json_examples) for some examples of this JSON data.
...
A `Type` will have other fields as defined in [Schema.fbs](https://github.com/apache/arrow/tree/main/format/Schema.fbs) depending on its name.
...
The `Field`’s “children” should be exactly two child fields. The first child must be named “run\_ends”, be non-nullable and be either an `int16`, `int32`, or `int64` type field. The second child must be named “values”, but can be of any type.
...
Archery Integration Test Cases[#](#archery-integration-test-cases "Permalink to this heading")
----------------------------------------------------------------------------------------------
...
There are two types of integration test cases: the ones populated on the fly by the data generator in the Archery utility, and _gold_ files that exist in the [arrow-testing](https://github.com/apache/arrow-testing/tree/master/data/arrow-ipc-stream/integration) repository.

### Data Generator Tests[#](#data-generator-tests "Permalink to this heading")
...
### Gold File Integration Tests[#](#gold-file-integration-tests "Permalink to this heading")

Pre-generated json and arrow IPC files (both file and stream format) exist in the [arrow-testing](https://github.com/apache/arrow-testing) repository in the `data/arrow-ipc-stream/integration` directory. These serve as _gold_ files that are assumed to be correct for use in testing. They are referenced by `runner.py` in the code for the [Archery](about:blank/developers/continuous_integration/archery.html#archery) utility. Below are the test cases which are covered by them:
