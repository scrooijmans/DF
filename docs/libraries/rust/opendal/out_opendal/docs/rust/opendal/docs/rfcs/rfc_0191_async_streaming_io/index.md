# Module rfc_0191_async_streaming_io Copy item path

<a href="https://opendal.apache.org/docs/rust/src/opendal/docs/rfcs/mod.rs.html#50" class="src">Source</a>
...
## <a href="https://opendal.apache.org/docs/rust/opendal/docs/rfcs/rfc_0191_async_streaming_io/index.html#summary" class="doc-anchor">Â§</a>Summary

Use `Stream`/`Sink` instead of `AsyncRead` in `Accessor`.

## <a href="https://opendal.apache.org/docs/rust/opendal/docs/rfcs/rfc_0191_async_streaming_io/index.html#motivation" class="doc-anchor">Â§</a>Motivation

`Accessor` intends to be the `underlying trait of all backends for implementers`. However, itâ€™s not so underlying enough.

### <a href="https://opendal.apache.org/docs/rust/opendal/docs/rfcs/rfc_0191_async_streaming_io/index.html#over-wrapped" class="doc-anchor">Â§</a>Over-wrapped
...
### <a href="https://opendal.apache.org/docs/rust/opendal/docs/rfcs/rfc_0191_async_streaming_io/index.html#inconsistent" class="doc-anchor">Â§</a>Inconsistent

OpenDALâ€™s `read` and `write` behavior is not consistent.
...
For `read`, OpenDAL returns a `BoxedAsyncReader` which users can decide when and how to read data. But for `write`, OpenDAL accepts a `BoxedAsyncReader` instead, in which users canâ€™t control the writing logic. How large will the writing buffer size be? When to call `flush`?

### <a href="https://opendal.apache.org/docs/rust/opendal/docs/rfcs/rfc_0191_async_streaming_io/index.html#service-native-optimization" class="doc-anchor">Â§</a>Service native optimization

OpenDAL knows more about the service detail, but returning `BoxedAsyncReader` makes it canâ€™t fully use the advantage.

For example, most object storage services use HTTP to transfer data which is TCP stream-based. The most efficient way is to return a full TCP buffer, but users donâ€™t know about that. First, users could have continuous small reads on stream. To overcome the poor performance, they have to use `BufReader`, which adds a new buffering between reading. Then, users donâ€™t know the correct (best) buffer size to set.
...
- Users who donâ€™t care about the behavior can use OpenDAL provided Reader, which always adopts the best optimization.

## <a href="https://opendal.apache.org/docs/rust/opendal/docs/rfcs/rfc_0191_async_streaming_io/index.html#guide-level-explanation" class="doc-anchor">Â§</a>Guide-level explanation
...
## <a href="https://opendal.apache.org/docs/rust/opendal/docs/rfcs/rfc_0191_async_streaming_io/index.html#reference-level-explanation" class="doc-anchor">Â§</a>Reference-level explanation
...
For fs, itâ€™s simple to implement `Stream` and `Sink` for `tokio::fs::File`.
...
## <a href="https://opendal.apache.org/docs/rust/opendal/docs/rfcs/rfc_0191_async_streaming_io/index.html#drawbacks" class="doc-anchor">Â§</a>Drawbacks

### <a href="https://opendal.apache.org/docs/rust/opendal/docs/rfcs/rfc_0191_async_streaming_io/index.html#performance-regression-on-fs" class="doc-anchor">Â§</a>Performance regression on fs
...
## <a href="https://opendal.apache.org/docs/rust/opendal/docs/rfcs/rfc_0191_async_streaming_io/index.html#rationale-and-alternatives" class="doc-anchor">Â§</a>Rationale and alternatives

### <a href="https://opendal.apache.org/docs/rust/opendal/docs/rfcs/rfc_0191_async_streaming_io/index.html#performance-for-switching-from-reader-to-stream" class="doc-anchor">Â§</a>Performance for switching from Reader to Stream
...
### <a href="https://opendal.apache.org/docs/rust/opendal/docs/rfcs/rfc_0191_async_streaming_io/index.html#performance-for-the-extra-channel-in-write" class="doc-anchor">Â§</a>Performance for the extra channel in `write`
...
### <a href="https://opendal.apache.org/docs/rust/opendal/docs/rfcs/rfc_0191_async_streaming_io/index.html#add-complexity-on-the-services-side" class="doc-anchor">Â§</a>Add complexity on the services side

Returning `Stream` and `Sink` make it complex to implement. At first glance, it does. But in reality, itâ€™s not.
...
Other helper functions will be covered at the Object-level which services donâ€™t need to bother.

## <a href="https://opendal.apache.org/docs/rust/opendal/docs/rfcs/rfc_0191_async_streaming_io/index.html#prior-art" class="doc-anchor">Â§</a>Prior art

### <a href="https://opendal.apache.org/docs/rust/opendal/docs/rfcs/rfc_0191_async_streaming_io/index.html#returning-a-writer" class="doc-anchor">Â§</a>Returning a `Writer`
...
But it only fixes the `Inconsistent` concern and canâ€™t help with other issues.

### <a href="https://opendal.apache.org/docs/rust/opendal/docs/rfcs/rfc_0191_async_streaming_io/index.html#slice-based-api" class="doc-anchor">Â§</a>Slice based API
...
The problem is `Accessor` doesnâ€™t have states:

- If we require all data must be passed at one time, we canâ€™t support large files read & write
- If we allow users to call `read`/`write` multiple times, we need to implement another `Reader` and `Writer` alike logic.

### <a href="https://opendal.apache.org/docs/rust/opendal/docs/rfcs/rfc_0191_async_streaming_io/index.html#accept-reader-and-writer" class="doc-anchor">Â§</a>Accept `Reader` and `Writer`

Itâ€™s also possible to accept `Reader` and `Writer` instead.
...
This API design addressed all concerns but made it hard for users to use. Primarily, we canâ€™t support `futures::AsyncRead` and `tokio::AsyncRead` simultaneously.

For example, we canâ€™t accept a `Box::new(Vec::new())`, user canâ€™t get this vec from OpenDAL.

## <a href="https://opendal.apache.org/docs/rust/opendal/docs/rfcs/rfc_0191_async_streaming_io/index.html#unresolved-questions" class="doc-anchor">Â§</a>Unresolved questions

None.

## <a href="https://opendal.apache.org/docs/rust/opendal/docs/rfcs/rfc_0191_async_streaming_io/index.html#future-possibilities" class="doc-anchor">Â§</a>Future possibilities
