...
///
/// # Arguments
/// _ `g`: graph with no negative cycle.
/// _ `source`: the source node.
///
/// # Returns
/// _ `Ok`: (if graph contains no negative cycle) a struct [`Paths`] containing distances and
/// predecessors along each shortest path. The vectors in [`Paths`] are indexed by the graph's node indices.
/// _ `Err`: if graph contains negative cycle.
///
/// # Complexity
/// _ Time complexity: **O(|V||E|)**.
/// _ Auxiliary space: **O(|V|)**.
///
