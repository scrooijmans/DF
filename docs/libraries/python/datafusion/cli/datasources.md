# Local Files / Directories — Apache DataFusion documentation
...
To read from an AWS S3 or GCS, use `s3` or `gs` as a protocol prefix. For example, to read a file in an S3 bucket named `my-data-bucket` use the URL `s3://my-data-bucket`and set the relevant access credentials as environmental variables (e.g. for AWS S3 you can use `AWS_ACCESS_KEY_ID` and `AWS_SECRET_ACCESS_KEY`).
...
See the [`CREATE EXTERNAL TABLE`](#create-external-table) section below for additional configuration options.

## `CREATE EXTERNAL TABLE`

It is also possible to create a table backed by files or remote locations via `CREATE EXTERNAL TABLE` as shown below. Note that DataFusion does not support wildcards (e.g. `*`) in file paths; instead, specify the directory path directly to read all compatible files in that directory.
...
Although wildcards (e.g., _.parquet or \*\*/_.parquet) may work for local filesystems in some cases, they are not supported by DataFusion CLI. This is because wildcards are not universally applicable across all storage backends (e.g., S3, GCS). Instead, DataFusion expects the user to specify the directory path, and it will automatically read all compatible files within that directory.
...
When specifying a directory path that has a Hive compliant partition structure, by default, DataFusion CLI will automatically parse and incorporate the Hive columns and their values into the table’s schema and data. Given the following remote object paths:
...
You can specify additional options for parquet files using the `OPTIONS` clause. For example, to read and write a parquet directory with encryption settings you could use:
...
For more details on the available options, refer to the Rust [TableParquetOptions](https://docs.rs/datafusion/latest/datafusion/common/config/struct.TableParquetOptions.html) documentation in DataFusion.
...
DataFusion CLI supports configuring [AWS S3](https://aws.amazon.com/s3/) via the `CREATE EXTERNAL TABLE` statement and standard AWS configuration methods (via the [`aws-config`](https://docs.rs/aws-config/latest/aws_config/) AWS SDK crate).

To create an external table from a file in an S3 bucket with explicit credentials:
...
If no credentials are specified, DataFusion CLI will use unsigned requests to S3, which allows reading from public buckets.

Supported configuration options are:

- Environment Variable: AWS_ACCESS_KEY_ID
  - Configuration Option: aws.access_key_id
  - Description:
- Environment Variable: AWS_SECRET_ACCESS_KEY
  - Configuration Option: aws.secret_access_key
  - Description:
- Environment Variable: AWS_DEFAULT_REGION
  - Configuration Option: aws.region
  - Description:
- Environment Variable: AWS_ENDPOINT
  - Configuration Option: aws.endpoint
  - Description:
- Environment Variable: AWS_SESSION_TOKEN
  - Configuration Option: aws.token
  - Description:
- Environment Variable: AWS_CONTAINER_CREDENTIALS_RELATIVE_URI
  - Configuration Option:
  - Description: See IAM Roles
- Environment Variable: AWS_ALLOW_HTTP
  - Configuration Option:
  - Description: If “true”, permit HTTP connections without TLS
- Environment Variable: AWS_SKIP_SIGNATURE
  - Configuration Option: aws.skip_signature
  - Description: If “true”, does not sign requests
- Environment Variable:
  - Configuration Option: aws.nosign
  - Description: Alias for skip_signature
...
- Environment Variable: GOOGLE_SERVICE_ACCOUNT
  - Configuration Option: gcp.service_account_path
  - Description: location of service account file
- Environment Variable: GOOGLE_SERVICE_ACCOUNT_PATH
  - Configuration Option: gcp.service_account_path
  - Description: (alias) location of service account file
- Environment Variable: SERVICE_ACCOUNT
  - Configuration Option: gcp.service_account_path
  - Description: (alias) location of service account file
- Environment Variable: GOOGLE_SERVICE_ACCOUNT_KEY
  - Configuration Option: gcp.service_account_key
  - Description: JSON serialized service account key
- Environment Variable: GOOGLE_APPLICATION_CREDENTIALS
  - Configuration Option: gcp.application_credentials_path
  - Description: location of application credentials file
- Environment Variable: GOOGLE_BUCKET
  - Configuration Option:
  - Description: bucket name
- Environment Variable: GOOGLE_BUCKET_NAME
  - Configuration Option:
  - Description: (alias) bucket name
