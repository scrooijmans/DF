...
* Component: Name
  * Description: Each type has a name. A type is expressed by providing a name. This name can be expressed in arbitrary case (e.g. varchar and vArChAr are equivalent) although lowercase is preferred.
  * Required: 
* Component: Nullability indicator
  * Description: A type is either non-nullable or nullable. To express nullability, a question mark is added after the type name (before any parameters).
  * Required: Optional, defaults to non-nullable
* Component: Variation
  * Description: When expressing a type, a user can define the type based on a type variation. Some systems use type variations to describe different underlying representations of the same data type. This is expressed as a bracketed integer such as [2].
  * Required: Optional, defaults to [0]
* Component: Parameters
  * Description: Compound types may have one or more configurable properties. The two main types of properties are integer and type properties. The parameters for each type correspond to a list of known properties associated with a type as declared in the order defined in the type specification. For compound types (types that contain types), the data type syntax will include nested type declarations. The one exception is structs, which are further outlined below.
  * Required: Required where parameters are defined
