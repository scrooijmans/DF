<script lang="ts">
	...
	interface NavItem {
		label: string
		href: string
		icon: 'settings' | 'users' | 'storage' | 'sync' | 'user' | 'palette' | 'logout'
	}
	...
	const { sections, onLogout }: { sections: NavSection[]; onLogout?: () => void } = $props()
...
<aside class="border-border flex w-64 flex-col border-r">
	...
	{#if onLogout}
		<div class="border-border border-t p-4">
			<button
				onclick={onLogout}
				class="text-muted-foreground hover:bg-secondary hover:text-foreground flex w-full items-center gap-3 rounded-md px-3 py-2 text-sm transition-colors"
			>
				<svg class="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
					<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 16l4-4m0 0l-4-4m4 4H7m6 4v1a3 3 0 01-3 3H6a3 3 0 01-3-3V7a3 3 0 013-3h4a3 3 0 013 3v1" />
				</svg>
